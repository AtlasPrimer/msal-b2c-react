...
function authCallback(errorDesc, token, error, tokenType) {
  if (errorDesc && errorDesc.indexOf('AADB2C90118') > -1) {
    ...
    // To solve Apple twice sign-in issue.
    sessionStorage.setItem('accessTokenForApple', token);
  }
}
...
function loginAndAcquireToken(successCallback) {
  ...
  if (!user) {
    ...
    // To solve Apple twice sign-in issue.
    var isApple = state.idToken.idp === 'https://appleid.apple.com';
    if (!state.accessToken && isApple) {
      var accessTokenForApple = sessionStorage.getItem('accessTokenForApple');
      if (accessTokenForApple) {
        state.accessToken = accessTokenForApple;
        state.noScopes = true;
      }
    }
...
var authentication = {
  ...
  signOut: function signOut() {
    // To solve Apple twice sign-in issue.
    state.noScopes = false;
    sessionStorage.removeItem('accessTokenForApple');
